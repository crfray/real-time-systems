...
public:
    void lock() {
        ...
        while (!locked.compare_exchange_strong(expected, 1)) {
            expected = 0;  // сброс ожидаемого значения после неудачи
    ...
    // Запрет копирования
    NewMutex(const NewMutex&) = delete;
    NewMutex& operator=(const NewMutex&) = delete;
    NewMutex() = default;
};

NewMutex table_mutex; // собственный
...
void cook_run() {
    ...
    while (!stop_simulation) {
        {
            lock_guard<NewMutex> lock(table_mutex); //собственный
